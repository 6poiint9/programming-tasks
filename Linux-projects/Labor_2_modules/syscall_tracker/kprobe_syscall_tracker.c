#include <linux/module.h> // basic modul operationen 
#include <linux/kernel.h>
#include <linux/init.h>
#include <linux/kprobes.h> // um hooks zu machen 
#include <linux/proc_fs.h> // um mit /pric zu interagieren 
...
// counter variable (static so that ne name collision)
...
// Procfs entry reference 
static struct proc_dir_entry *proc_entry;

// Kprobes declaration -> devlare where hook is places 
...
// Pre handlers to count + increment (while checking if max)
...
// wrapper + single_open to open file
...
// defines how /proc file should beahve 
...
static int __init syscall_tracker_init(void)
{
    ...
    return 0;
// error handling -> unregister a kprobe that was previously registered 
// if a registration fails unregister all hooks 
...
// Exit -> unregister hooks and removes /proc file/entry 
// logs final output to dmesg | tail 
